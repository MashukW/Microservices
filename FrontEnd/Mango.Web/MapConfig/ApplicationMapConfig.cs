    ...
    public class ApplicationMapConfig : Profile
    {
        ...
        private void MapCheckoutMembers()
        {
            CreateMap<CheckoutView, CheckoutApi>()
                .ForMember(dest => dest.CouponCode, src => src.MapFrom(x => x.Cart.CouponCode))
                .ForMember(dest => dest.CartItems, src => src.MapFrom(x => x.Cart.CartItems))
                .ForMember(dest => dest.DiscountAmount, src => src.MapFrom(x => x.Cart.DiscountAmount))
                .ForMember(dest => dest.TotalCost, src => src.MapFrom(x => x.Cart.TotalCost))
